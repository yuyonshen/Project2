...
public class StudentDAO {
    ...
    public static Student queryById(int id) {
        ...
        try {
            c = DBUtil.getConnection();
            String sql = "select s.id ,s.student_name,s.student_graduate_year,s.student_major,s.student_email,s.dorm_id,s.create_time,b.id building_id,b.building_name from student s join dorm d on s.dorm_id=d.id join building b on d.building_id=b.id where s.id=?";
            ...
            while (rs.next()) {
                ...
                s.setStudentEmail(rs.getString("student_email"));
                s.setDormId(rs.getInt("dorm_id"));
                s.setCreateTime(new Date(rs.getTime("create_time").getTime()));
                s.setBuildingId(rs.getInt("building_id"));
                s.setBuildingName(rs.getString("building_name"));
    ...
    public static int apply(Student s) {
        ...
        try {
            ...
            throw new RuntimeException("分配学生宿舍详情出错", e);
        } finally {
            DBUtil.close(c, ps);
        }
    }
